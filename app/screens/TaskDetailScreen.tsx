...
const TaskDetailScreen = () => {
  ...
  const handleUpdateTask = () => {
    ...
    Alert.alert('Task Updated', 'The task details have been updated successfully.');
  };
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      ...
      {['to do', 'in progress', 'done'].map((s) => (
        <TouchableOpacity
          key={s}
          className={`p-2 mb-2 rounded ${status === s ? 'bg-blue-500' : 'bg-gray-200'}`}
          onPress={() => setStatus(s)}
        >
          <Text className={`text-center ${status === s ? 'text-white' : 'text-black'}`}>{s.charAt(0).toUpperCase() + s.slice(1)}</Text>
        </TouchableOpacity>
      ))}
      <TouchableOpacity
